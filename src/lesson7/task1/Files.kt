...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    for (i in substrings) strings[i] = 0
    ...
    for (line in input.readLines()) {
        ...
        for (substring in strings.keys) {

            first = 0
            val substringEdited = substring.toLowerCase()
            count = lLine.indexOf(substringEdited, first)
            while (count > -1) {

                strings[substring] = strings[substring]!!.plus(1)
                first = count + 1
                count = lLine.indexOf(substringEdited, first)
...
fun sibilants(inputName: String, outputName: String) {
    val sibSet = setOf('ж', 'ч', 'ш', 'щ')
    val lettersAssigned = mapOf('ы' to "и", 'я' to "а", 'ю' to "у", 'Я' to "А", 'Ю' to "У", 'Ы' to "И")
    ...
    File(outputName).bufferedWriter().use {
        ...
        for (line in File(inputName).readLines()) {
            ...
            for (i in 1 until line.length)
                if (line[i - 1].toLowerCase() in sibSet && line[i] in lettersAssigned)
                    it.write(lettersAssigned.getValue(line[i]))
